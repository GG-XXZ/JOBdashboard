...
Use this reference when initializing JOBdashboard for a new user or refreshing their job-search operating system.
...
- Voluntary self-ID strategy: tell the user that if they do not choose, JOBdashboard will default to `Prefer not to say` / decline / leave blank when available.
...
JOBdashboard can use external fit signals from tools like Jobright or Simplify when the user already uses them. These tools are optional, not required.
...
- If the user does not choose a strategy, explicitly tell them JOBdashboard will default to non-disclosure and will not guess identity information.
