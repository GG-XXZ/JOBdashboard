...
JOBdashboard is a second-generation adaptation of the original `DanielPan12/JobHuntBot` repository, authored and maintained by DanielPan12. The source repository references Yvonne He's ApplyPilot workflow as an upstream source; this package preserves that attribution while extending the dashboard, source pools, resume queue, email reminders, and launcher. It helps users turn job searching into a repeatable system: profile, dashboard, screening rules, resume strategy, application execution, blocker triage, and follow-up.
...
- The user is installing JOBdashboard for the first time.
...
- Read `dashboard/lead_sources.csv` before each search round. Treat every `Active` public-account name, online-document URL, or other source as an additional search/work-pool input alongside proactive web search. A source is not itself a job lead: extract and verify specific roles before adding them to `job_pool.csv`, record the actual source, and never mark a role open solely because it appeared in an imported document.
...
For per-role tailoring, use `dashboard/resume_templates.csv` and `dashboard/resume_requests.csv`:

- Process `Queued` resume requests before opening that role's application form. Read the exact JD, source resume, and `experience_bank.md`; change status through `Generating` to `Generated` (or `Failed` with a truthful reason).
- Save outputs under `tailored-resumes/<company>/`. The company may appear in the directory name but must not appear in the resume filename; use a role-based name such as `结构仿真工程师-定制简历.pdf`.
- Preserve the selected template's visual hierarchy, keep an editable version plus PDF where practical, and never overwrite the source resume.
- When an application offers a resume attachment field, prefer the exact `Generated` PDF for that company + job title, verify the displayed filename and parsed fields, and record the path in `application_log.csv` after submission.
...
When a form is completely filled but final consent has not been given, set the job to `Ready to submit`, keep the final page available when possible, and continue completing other queued applications. For this project, always open or preserve the corresponding application page in a visible browser when a job reaches `Ready to submit` or otherwise needs the user's submission confirmation; open multiple waiting jobs in separate tabs. Opening a page is only a handoff and never authorizes the final click. `Ready to submit` is a waiting queue, not proof of submission and not authorization to click. A user may also manually confirm an application they completed outside the Agent; in that case record the user's confirmation/evidence in a new `Submitted` application-log row instead of claiming browser verification.
...
- `Ready to submit`: the form is complete and only the user's explicit final-submit confirmation remains. It stays visible in the pending area and does not count as submitted.

The dashboard may also import a job the user already submitted independently. Deduplicate it by URL or exact company + job title, set `job_pool.status=Submitted`, append an `application_log` row whose evidence clearly says it was user-confirmed/imported, and never describe it as Agent-verified unless confirmation evidence was actually inspected.
...
After each run, summarize blockers and convert repeated issues into rules. JOBdashboard should improve through use: address matching, dropdown handling, resume upload checks, account/session checks, and ATS-specific lessons belong in the dashboard and rules.
...
Do not publish or copy private resumes, phone numbers, emails, addresses, immigration documents, application history, browser sessions, cookies, OTPs, or user-specific secrets into a public JOBdashboard package.
